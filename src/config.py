...
class Arguments(TrainingArguments):
    ...
    prefix_embed_file: str = field(
        default='embedding_layer.pth',
    )
    ...
    topk_as_positive: List[int] = field(default_factory=lambda: [0, 1], metadata={'help': 'candidates with top-k scores as positive'})
    # bottomk_as_negative: int = field(default=-1, metadata={'help': 'candidates with bottom-k scores as negative'})
    bottomk_as_negative: List[int] = field(default_factory=lambda: [-1], metadata={'help': 'candidates with bottom-k scores as positive'})

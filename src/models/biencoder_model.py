...
class BiencoderModel(nn.Module):
    ...
    def forward(self, batch_dict: Dict[str, Tensor]) -> BiencoderOutput:
        ...
        if not self.args.do_kd_biencoder:
            ...
            group_log_scores = torch.log_softmax(group_scores, dim=-1) #作为retriever distribution
            kd_log_target = torch.log_softmax(batch_dict['kd_labels'], dim=-1)#作为reward model distribution

            kd_loss = self.kl_loss_fn(input=group_log_scores, target=kd_log_target)
            ce_loss = self.cross_entropy(scores, labels)#对比学习的InfoNCE就是需要先生成labels，再用cross_entropy计算
    ...
    def _compute_scores(self, batch_dict: Dict[str, Tensor]) -> Tuple:
        embeds = self._encode(self.lm_p, batch_dict)#使用self.lm_p作为encoder，对input_ids和attention_mask进行编码
        batch_size = batch_dict['input_ids'].shape[0] // (self.args.train_n_passages + 1) #train_n_passages（docs数）+自身
        q_reps = embeds[:batch_size]#query的表示
        p_reps = embeds[batch_size:]#docs的表示
        assert p_reps.shape[0] == q_reps.shape[0] * self.args.train_n_passages

        all_q_reps = dist_gather_tensor(q_reps) #[32,768] -> [256,768] 各个进程的tensor收集起来
        all_p_reps = dist_gather_tensor(p_reps) #[128,768] -> [1024,768]
        ...
        if self.args.l2_normalize:
            all_scores = all_scores / self.args.t #除以温度系数进行归一化
        ...
        scores = all_scores.index_select(dim=0, index=local_query_indices) #local query的scores和labels
    ...
    def _encode(self, encoder: PreTrainedModel, input_dict: dict) -> Optional[torch.Tensor]:
        ...
        embeds = pool(last_hidden_states=outputs.last_hidden_state,
                      ...
                      pool_type=self.args.pool_type)#avg pool  [160, 768]
        if self.args.l2_normalize:
            embeds = F.normalize(embeds, dim=-1)#最后一个维度上进行L2标准化  [160, 768]
        return embeds.contiguous()#转为连续的tensor
    ...
    def build(cls, args: Arguments, **hf_kwargs):
        ...
        lm_q = AutoModel.from_pretrained(args.model_name_or_path) #e5是bert model
        lm_p = lm_q

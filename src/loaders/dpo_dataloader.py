...
class DPODataset:
    ...
    def __init__(self, input_files: List[str], args: Arguments, 
        ...
        self.dataset.set_transform(self._transform_func_dpo)
    ...
    def _transform_func_dpo(self, examples: Dict[str, List]) -> Dict[str, List]:
        ...
        prompt = input_queries[0]
        positive_doc = input_docs[0]
        negative_doc = input_docs[-1]

        return {
            'prompt': prompt,
            'chosen': positive_doc,
            'rejected': negative_doc
        }
...
class DPODataLoader:
    ...
    def __init__(self, args: Arguments, tokenizer: PreTrainedTokenizerFast):
        ...
        # self.train_dataset = self._get_transformed_datasets() ###暂时注释
        self.dpo_dataset = self._get_transformed_datasets()
        self.train_dataset = self.dpo_dataset.dataset
    ...
    def _get_transformed_datasets(self) -> DPODataset:
        train_dataset = None
        # eval_dataset = None
        ...
        if self.args.train_file is not None:
            ...
            train_dataset = DPODataset( #latent_dataset
                args=self.args,

...
class BiencoderDataset(torch.utils.data.Dataset):
    def __init__(self, args: Arguments,
        ...
        self.negative_size = args.train_n_passages - 1 #说明1个正样本
        ...
        # init_dataset: Dataset = load_dataset('json', data_files=self.input_files, split='train')
        # num_samples = len(init_dataset) // 100
        # self.dataset: Dataset = init_dataset.select(random.sample(range(len(init_dataset)), num_samples))
        ...
        with self.args.main_process_first(desc="pre-processing"):
            ...
            self.dataset = self.dataset.map(
                partial(to_positive_negative_format,
                        topk_as_positive=args.topk_as_positive,#3
                        bottomk_as_negative=args.bottomk_as_negative),#16
                load_from_cache_file=args.world_size > 1,
                desc='to_positive_negative_format',
                remove_columns=['doc_ids', 'doc_scores']
            )
            self.dataset = self.dataset.map(
                partial(_filter_out_same_label_negatives, self.corpus),#过滤掉文本分类任务的一些不合格负样本
                load_from_cache_file=args.world_size > 1,
    ...
    def _transform_func(self, examples: Dict[str, List]) -> Dict[str, List]:
        examples = deepcopy(examples) #每个query对应3个正样本，16个负样本
        ...
        input_doc_ids: List[int] = group_doc_ids( #4个，第1个是正样本，后3个是负样本
            examples=examples,
        ...
        if self.args.do_kd_biencoder:
            ...
            #qid_to_doc_id_to_score 将正负样本的id和score写为字典形式，前3个是正样本，后16个是负样本
            batch_dict['kd_labels'] = []
            for idx in range(0, len(input_doc_ids), step_size):
                qid = examples['query_id'][idx // step_size] #query id

...
class KTODataset_theta_lora_multi_tasks(Dataset):
    def __init__(self, args: Arguments, split: str = 'train'):
        ...
        # self.dataset = self.dataset.filter(lambda x: x['task_name'] in ["snli", "wsc", "qqp", "mrpc", "arc_c", "paws", "arc_e", "ag_news", "rte", "boolq", "hellaswag",  "qnli", "winogrande", "sst2", "piqa", "copa", "openbookqa"]) #args.llm_eval_tasks  "sentiment140", "yelp",  "mnli",
        ...
        # self.args.max_train_samples = 10000
    ...
    def _transform_func_kto_trainer(self, batch_examples: Dict[str, List]) -> Dict[str, List]:
        ...
        for i in range(len(batch_examples['query'])):
            ...
            right_answer = batch_examples['answers'][i][0]
            theta_and_x = theta_description + '\n' +  query
            new_prompts.append(theta_and_x)
            new_completions.append(right_answer)
            new_labels.append(True)

            possible_answers = get_possible_answers_by_task_name(task_name)
            if possible_answers == None:  #generaton tasks
                assert len(batch_examples['answers']) > 1
                random_index = random.choice([idx for idx in range(len(batch_examples['answers'])) if idx != i])
                wrong_answer = batch_examples['answers'][random_index][0]
            else: #classification tasks
                wrong_answers = [answer for answer in possible_answers if answer != right_answer]
                wrong_answer = random.choice(wrong_answers)
            new_prompts.append(theta_and_x)
            new_completions.append(wrong_answer)
            new_labels.append(False)

            for pos_id in pos_doc_ids:
                chosen_text = self.corpus[int(pos_id)]['contents'].strip()
                demo_and_x = chosen_text + '\n' + query
                new_prompts.append(demo_and_x)
                new_completions.append(theta_description)
                new_labels.append(True)

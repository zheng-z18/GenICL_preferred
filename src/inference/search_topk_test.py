...
def main():
    ...
    def _map_func(example: Dict, idx: int) -> Dict: #example包含query，options，answers，task_name，query_id
        score_doc_ids: List[Tuple[float, str]] = topk_score_doc_ids[idx]#idx表示当前query的索引，获取当前query对应检索出的docs
        # Exclude the example itself from the top-k candidates.
        score_doc_ids = [t for t in score_doc_ids if not all_contents[int(t[1])].startswith(example['query'])] #排除corpus中以当前query开头的文本，即排除自身。由100个变成99个
        #score_doc_ids中第一个也是得分最高的是自己，被去掉了
    ...
    dataset = dataset.map(_map_func, with_indices=True, num_proc=1, desc='Add top-k candidates') #num_proc=1 使用单个进程来处理
    dataset = dataset.filter(lambda example: len(example['doc_ids']) > 1)#要求检索出来的doc大于1个

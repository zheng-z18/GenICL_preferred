...
def main():
    ...
    for task_name in args.llm_eval_tasks:# args.llm_eval_tasks,,real_llm_eval_tasks
        ...
        demo_query_sentences: List[str] = []
        demo_query_description_sentences: List[str] = []
        ...
        # for query, doc_ids in zip(task_ds['query'], task_ds['doc_ids']):
            ...
            for doc_id in doc_ids:
                ...
                demo_query_description_sentences.append(single_demo_query_description)
        demo_query_perplexity: List[float] = llm.calculate_perplexity(demo_query_sentences)
        demo_query_description_perplexity: List[float] = llm.calculate_perplexity(demo_query_description_sentences)
        assert len(demo_query_perplexity) == len(demo_query_description_perplexity)
        score_perplexity = [x / y for x, y in zip(demo_query_description_perplexity, demo_query_perplexity)]
        ...
        logger.info('score_perplexity length: {}'.format(len(score_perplexity)))
        ...
        for i, doc_ids in enumerate(tqdm(dataset_doc_ids, desc='reconstruct doc_scores')):
            ...
            #     break
            assert start_idx < len(score_perplexity)
            dataset_doc_scores.append(score_perplexity[start_idx:end_idx]) #获得检索到的各个样本的得分，
            start_idx = end_idx
        ...
        for ids, scores in zip(dataset_doc_ids, dataset_doc_scores):
            ...
            sorted_combined = sorted(combined, key=lambda x: x[1])#从小到大排序
            top_k = sorted_combined[:args.llm_k_shot] #取最小值

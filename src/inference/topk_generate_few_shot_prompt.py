...
def main():
    ...
    corpus: Dataset = load_dataset(
        'json', data_files='data/tasks/passages.jsonl.gz', split='train')
    # columns: query_id / query / answers / task_name
    eval_dataset: Dataset = load_dataset(
        'json', data_files='tools/check_docs/filter_test.jsonl.gz', split='train',download_mode='force_redownload')
    ...
    for task_name in args.llm_eval_tasks:# args.llm_eval_tasks,,real_llm_eval_tasks
        ...
        dataset_doc_scores: List[List[int]] = task_ds['doc_scores']
        topk_doc_ids = []
        topk_scores = []

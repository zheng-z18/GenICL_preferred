...
from model_utils import parse_model_id, build_extend_llm, build_llm
...
def _worker_gen_llm_scores():
    ...
    dataset = dataset.shard(num_shards=args.world_size, index=gpu_idx, contiguous=True) ##???为什么分块
...
def main():
    ...
    # )
    out_path: str = '{}/{}_{}.jsonl.gz'.format(
        args.output_dir, parse_model_id(args.llm_model_name_or_path), args.search_split
    )

...
def main():
    ...
    eval_dataset: Dataset = load_dataset('json', data_files=args.train_file, split='train')
    ...
    llm: BaseLLM = build_extend_llm(args, accelerator=accelerator)
    ...
    for task_name in args.llm_eval_tasks:# args.llm_eval_tasks,,real_llm_eval_tasks
        ...
        topk_doc_ids = []
        topk_scores = []
        available_ids = task_corpus['id']
        # print("random IDs:", random.sample(available_ids, 3))
        # debug_content = corpus[int(random.sample(available_ids, 1)[0])]['contents']
        # print("random content:", debug_content)
        for each_data in tqdm(task_ds, desc="Processing data", unit="item"):
            ...
            #     json.dump(debug_content, f, ensure_ascii=False, indent=4)

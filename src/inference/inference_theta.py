...
def task_template(task_name, current_query):
    if task_name == "sst2":
        q_prefix = "Review: "
        a_prefix = "Sentiment: "
    elif task_name == "sst5":
    ...
    new_query = q_prefix + current_query + "\n" + a_prefix
    return new_query
...
def main():
    ...
    for task_name in args.llm_eval_tasks:# args.llm_eval_tasks,,real_llm_eval_tasks
        ...
        for passage in task_corpus:
            ...
            content_query = task_template(task_name, content_query)
            
            if args.channel == True:
                content = content_answer + content_query #channel
            else:
                content = content_query + content_answer

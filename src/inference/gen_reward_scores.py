...
def _worker_gen_teacher_score():
    ...
    dataset = load_dataset('json', data_files=kd_gen_score_in_path, cache_dir='/public/home/wangyf7/DoraemonGate/50Tb/zheng_z/cache/huggingface/datasets')['train']
    if args.dry_run:
    ...
    for ex in tqdm.tqdm(dataset, desc='get qid-pid pairs', mininterval=3):
        for doc_id in ex['doc_ids']:#每个 doc_id 一行，其他和本query相关的都一样
    ...
    tokenizer: PreTrainedTokenizerFast = AutoTokenizer.from_pretrained(args.model_name_or_path)#'/public/home/wangyf7/DoraemonGate/50Tb/zheng_z/llm_retriever/baseline/llama-2-chat_reward_it1'
    ...
    for idx in range(len(query_ids)):#每个query_id 对应的 doc_id 和score

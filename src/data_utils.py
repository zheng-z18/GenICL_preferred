...
def to_positive_negative_format(example: Dict, topk_as_positive: int = 1, bottomk_as_negative: int = -1) -> Dict:
    ...
    sorted_indices: List[int] = np.argsort(example['doc_scores'])[::-1] #[::-1]表示降序排列
    ...
    #bottomk_as_negative <= 0 选择剩余所有样本的索引。否则选择最后bottomk_as_negative个样本作为负样本
    negative_indices = [idx for idx in negative_indices if idx not in positive_indices] #确保负样本索引列表中不包含正样本索引
...
def save_to_readable_format(in_path: str, corpus: Dataset, shuffle: bool = False, max_num_samples: int = 10000):
    ...
    dataset: Dataset = load_dataset('json', data_files=in_path, split='train', 

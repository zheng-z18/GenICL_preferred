...
class LatentTrainer(Trainer):
    ...
    def __init__(self, *pargs, **kwargs):
        super(LatentTrainer, self).__init__(*pargs, **kwargs)
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        #     if labels is None:
        #         raise ValueError("Labels are required for computing accuracy.")
        #     shift_logits = outputs.logits[..., :-1, :].contiguous()
        #     shift_labels = labels[..., 1:].contiguous()
        #     preds = torch.argmax(shift_logits, dim=-1)
        #     mask = shift_labels != -100
        #     correct = (preds == shift_labels) & mask
        #     correct_sum = correct.sum().float()
        #     total_sum = mask.sum().float()
        #     step_acc = correct_sum / total_sum if total_sum > 0 else torch.tensor(0.0)
        #     self.acc_meter.update(step_acc.item())

        #     if self.state.global_step > 0 and self.state.global_step % self.args.logging_steps == 0:
        #         avg_acc = self.acc_meter.avg
        #         logger.info(f"Step: {self.state.global_step}, {self.acc_meter}")

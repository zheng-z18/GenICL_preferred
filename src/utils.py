...
def dist_gather_tensor(x: Optional[torch.Tensor]) -> Optional[torch.Tensor]:
    ...
    x = x.contiguous()
    x_gather = Gather.apply(x) #将 x 在所有进程间收集
    x_gather = torch.cat(x_gather, dim=0) #在第0维度上拼接
    return x_gather
...
def select_grouped_indices(scores: torch.Tensor,
    ...
    assert len(scores.shape) == 2  #[32,1792] 确保是二维tensor
    batch_size = scores.shape[0] #32
    assert batch_size * group_size <= scores.shape[1]

    indices = torch.arange(0, group_size, dtype=torch.long) #[0,1,2,3]
    indices = indices.repeat(batch_size, 1) #沿1维度 重复batch_size次
...
def full_contrastive_scores_and_labels(query: torch.Tensor,#[256,768]
                                       key: torch.Tensor,#[1024,768]
    ...
    train_n_passages = key.shape[0] // query.shape[0] #每个query对应的docs的数量
    labels = torch.arange(0, query.shape[0], dtype=torch.long, device=query.device) #[0,1,2,3.....256]
    labels = labels * train_n_passages #[0,4,8....1024]

    # batch_size x (batch_size x n_psg)
    qk = torch.mm(query, key.t()) #[256,1024]

    # batch_size x dim
    sliced_key = key.index_select(dim=0, index=labels) #[256,768] 选择key中对应于labels的行出来
    assert query.shape[0] == sliced_key.shape[0]

    # batch_size x batch_size
    kq = torch.mm(sliced_key, query.t()) #[256,256]
    kq.fill_diagonal_(float('-inf')) #将对角线的值设为无穷大，防止自身相似度对计算的影响
...
def pool(last_hidden_states: Tensor,
    ...
    #根据attention_mask将last_hidden_states中需要掩盖的地方填充0
    if pool_type == "avg":
...
class DictTrie:#用于快速查找next token，限制生成的输出
    ...
    @staticmethod
    def _get_from_trie(
            prefix_sequence: List[int],#generated_ids.tolist()[input_len:]
            trie_dict: Dict,
    ):
        if len(prefix_sequence) == 0:#没有前缀限制，返回所有可能的next token id
            output = list(trie_dict.keys())
            return output
        elif prefix_sequence[0] in trie_dict:#检查 prefix_sequence 的第一个token ID 是否存在于当前 Trie 节点的子字典中
            return DictTrie._get_from_trie(
                prefix_sequence[1:], #传入剩下的前缀序列
                trie_dict[prefix_sequence[0]] #和对应子节点的 Trie 字典
            )
        else:
            return [] #如果前缀序列的第一个令牌 ID 不存在于当前 Trie 节点的子字典中，说明当前前缀序列在 Trie 中没有匹配的路径。
...
def build_trie(tokenizer: PreTrainedTokenizerFast, output_texts: List[str]) -> DictTrie:
    ...
    # newline_id = tokenizer.eos_token
    # output_texts = [f"{t}{newline_id}" for t in output_texts]
...
def wait_until_all_files_show_up(file_names: List[str]): #file_names需要检查的文件名列表
    ...
    while True:
        ...
        for file_name in file_names:
            if not os.path.exists(file_name):
                ...
                break
        if all_files_exist:
            time.sleep(30) #等待30s确保文件不仅存在而且完全写入
            return

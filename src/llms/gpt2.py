...
class GPT2(BaseLLM):
    ...
    def __init__(self, args: Arguments, model_name_or_path: str = 'gpt2-xl', **kwargs):
        ...
        # self.model: GPT2LMHeadModel = AutoModelForCausalLM.from_pretrained(model_name_or_path, device_map="auto")
        self.model: GPT2LMHeadModel = AutoModelForCausalLM.from_pretrained(model_name_or_path)
    ...
    def batch_decode(self, input_texts: List[str], prefix_trie=None, **kwargs) -> List[str]:
        ...
        for batch_dict in tqdm.tqdm(data_loader, mininterval=10, desc='batch decode'):
            ...
            with torch.amp.autocast('cuda') if self.args.fp16 else nullcontext():
                ...
                generated_token_ids = outputs.sequences[:, input_len:] #提取input_len之后的，也就是新生成的部分
    ...
    def batch_comparison_decode(self, input_texts: List[str], prefix_trie=None, **kwargs) -> List[str]:
        collator = DecodeCollator(
            tokenizer=self.tokenizer,
            max_length=self.args.llm_max_input_length,
            pad_to_multiple_of=8
